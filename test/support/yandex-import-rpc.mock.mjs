...
globalThis.fetch = async (url, options) => {
  ...
  if (body.p_action === 'read') {
    assert.equal(calls, 1);
    return Response.json({
      company_id: '13f3cb80-487a-4a19-96a1-fb3103200230',
      location_id: '9a95f63b-18e6-447b-a449-8530b67ddbae',
      external_org_id: '54309413522',
      state: 'ERROR', revision: 7
    });
  }
  assert.equal(calls, 1); assert.equal(body.p_action, 'replace'); assert.equal(body.p_expected_revision, 7);
  ...
  if (process.env.IMPORT_TEST_MODE === 'bad_result') return Response.json({ state: ['NOT_CONFIGURED'], revision: 8 });
  return Response.json({ state: 'NOT_CONFIGURED', revision: 8 });
};

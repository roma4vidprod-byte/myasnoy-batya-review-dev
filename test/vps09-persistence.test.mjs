...
async function fixture(fn,opt={}){
 ...
 try{
  ...
  const row={...encryptSession(scope,{account:ACCOUNT,cookies:[{name:'synthetic',value:secret,domain:'.yandex.ru',path:'/',secure:true,expires:-1}]},keyring,Date.now(),context),company_id:scope.companyId,location_id:scope.locationId,external_org_id:scope.organizationId,state:opt.state??'READY',revision:opt.revision??6};
...
test('VPS09 accepted mutable full batch -> existing writer once; owner reply preserved; no downstream',()=>fixture(async f=>{
 const r=await f.service.persistManual(scope,{expectedRevision:6});assert.equal(r.ok,true);assert.equal(r.persistence_result.inserted,71);
 ...
 assert.deepEqual(f.calls,[1,2,3,4]);assert.equal(f.row.revision,6);assert.doesNotMatch(JSON.stringify(r),new RegExp(secret));
}));
for(const opt of [{bad:true},{incomplete:true},{race:true},{networkFail:true},{state:'ERROR'},{revision:5}])test('VPS09 pre-persistence failure gives zero writer calls '+JSON.stringify(opt),()=>fixture(async f=>{
 const r=await f.service.persistManual(scope,{expectedRevision:6});assert.equal(r.ok,false);assert.equal(f.writes.length,0);assert.equal(r.review_persistence,'NOT_RUN');assert.doesNotMatch(JSON.stringify(r),new RegExp(secret));
},opt));
test('VPS09 DB uncertainty no retry and never successful batch',()=>fixture(async f=>{
 const r=await f.service.persistManual(scope,{expectedRevision:6});assert.equal(r.ok,false);assert.equal(r.error,'REVIEW_STORAGE_FAILED');assert.equal(f.writes.length,1);assert.equal(r.review_persistence,'ATTEMPTED');assert.doesNotMatch(JSON.stringify(r),new RegExp(secret));
},{dbFail:true}));
test('VPS09 wrong scope before provider/writer',()=>fixture(async f=>{
 const r=await f.service.persistManual({...scope,locationId:'11111111-1111-4111-8111-111111111111'},{expectedRevision:6});assert.equal(r.ok,false);assert.equal(f.calls.length,0);assert.equal(f.writes.length,0);
}));
test('VPS09 cannot use diagnostic options as a persistence bypass',()=>fixture(async f=>{
 const r=await f.service.contractDiagnosticFull(scope,{expectedRevision:6,paginationMode:'MUTABLE_OFFSET',persist:true});assert.equal(r.ok,false);assert.equal(f.writes.length,0);assert.equal(f.calls.length,0);
}));
test('VPS09A real service/transport preserves safe network cause; writer stays NOT_RUN',()=>fixture(async f=>{
 const r=await f.service.persistManual(scope,{expectedRevision:6});assert.equal(r.error,'YANDEX_NETWORK_ERROR');
...
test('VPS09 current revision is not hardcoded to 4 or 6',()=>fixture(async f=>{
 const r=await f.service.persistManual(scope,{expectedRevision:9});
 assert.equal(r.ok,true);assert.equal(f.writes.length,1);assert.equal(f.row.revision,9);
},{revision:9}));

...
export async function diagnoseContractDrift(input,generate){
  ...
  if(result.classification==='NO_DRIFT'&&result.action_id!=='KEEP_BLOCKED')
    fail('AI_CONTRACT_DRIFT_OUTPUT_INVALID');
  if(['PAGE_NAVIGATION_CHAIN_DRIFT','PAGE_PATH_DRIFT','PRELOAD_CONTRACT_DRIFT','CSRF_CONTRACT_DRIFT',
      'API_CONTRACT_DRIFT','BROWSER_NAVIGATION_DRIFT'].includes(result.classification)&&result.action_id!=='CONTRACT_REVIEW')
    fail('AI_CONTRACT_DRIFT_OUTPUT_INVALID');

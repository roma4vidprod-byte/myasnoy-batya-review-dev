...
export function replyApprovalFingerprint({
  ...
  const values=[
    uuid(actionId,'REPLY_APPROVAL_ACTION_INVALID'),
    text(reviewId,256,'REPLY_APPROVAL_REVIEW_INVALID'),
    text(replyText,2500,'REPLY_APPROVAL_TEXT_INVALID'),
    uuid(companyId,'REPLY_APPROVAL_SCOPE_INVALID'),
    uuid(locationId,'REPLY_APPROVAL_SCOPE_INVALID'),
    text(provider,32,'REPLY_APPROVAL_PROVIDER_INVALID')
  ];
  if(values[5]!=='yandex')fail('REPLY_APPROVAL_PROVIDER_INVALID');
  const payload=values.map(value=>Buffer.byteLength(value,'utf8')+':'+value).join('');

...
export function createYandexReplyTransport({
  ...
  return async function send({externalReviewId,replyText,authorization}={}){
    ...
    const csrfRequest=buildYandexCsrfCandidate();
    let bootstrap;
    try{
      bootstrap=await fetchImpl(csrfRequest.url,{
        method:csrfRequest.method,
        headers:{...csrfRequest.headers,Cookie:cookies},
        redirect:'manual',signal:AbortSignal.timeout(10000)
      });
      providerRequests++;
    }catch{replyFail('YANDEX_REPLY_CSRF_BOOTSTRAP_NETWORK');}
    const bootstrapText=await boundedText(bootstrap);
    const csrfResult=classifyYandexCsrfResponse({
      status:bootstrap.status,
      contentType:bootstrap.headers.get('content-type')||'',
      text:bootstrapText,
      location:bootstrap.headers.get('location')
    });
    if(!csrfResult.ok){
      const mapped={
        YANDEX_CSRF_HTTP_401:'YANDEX_REPLY_HTTP_401',
        YANDEX_CSRF_HTTP_403:'YANDEX_REPLY_HTTP_403',
        YANDEX_CSRF_REDIRECT:'YANDEX_REPLY_REDIRECT',
        YANDEX_CSRF_RATE_LIMITED:'YANDEX_REPLY_RATE_LIMITED'
      }[csrfResult.code]||'YANDEX_REPLY_CSRF_BOOTSTRAP_DRIFT';
      replyFail(mapped);
    }

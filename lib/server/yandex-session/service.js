...
import { scopeOf, encryptSession, decryptSession, decryptSessionForRequest, serverOnly, fail, FAILURE_CODES } from './crypto.js';
...
export function createYandexSessionService({ store, keyring, allowRead = false, fetchImpl, notify = sendSessionAlert, now = () => new Date(), persistenceWriter = null, context, allowManualPersistence = false }) {
  serverOnly(context);
  if (context && persistenceWriter && allowManualPersistence !== true) fail('SESSION_MODE_INVALID');
  ...
  async function run(input, { mode = 'health', pageBase, maxPages = 20, safeReport = false, persistenceExpected = 'any' } = {}) {
    ...
    try {
      ...
      const session = decryptSessionForRequest(scope, row, keyring, now().getTime(), context);
  ...
  async function completeRead(input, { expectedRevision, paginationMode = 'STRICT_LEGACY' } = {}, persist = false) {
    serverOnly(context);
    const scope = scopeOf(input);
    const mutable = paginationMode === 'MUTABLE_OFFSET';
    const maxPages = mutable ? 10 : 5;
    const requiredState = mutable && !persist ? 'ERROR' : 'READY';
    ...
    let persistenceStatus = 'NOT_RUN', persistenceResult = null, networkDiagnostic = null;
    ...
    const summary = () => ({operation:'contract_diagnostic_full', ...noEffects,
      ...
      ...(persist?{operation:'manual_persistence',review_persistence:persistenceStatus,persistence_result:persistenceResult}: {})});
    try {
      if (!['STRICT_LEGACY','MUTABLE_OFFSET'].includes(paginationMode) ||
          (mutable && (!context || (!persist && expectedRevision !== 3)))) fail('SESSION_MODE_INVALID');
      if(persist && (!mutable || allowManualPersistence !== true || !persistenceWriter))fail('SESSION_MODE_INVALID');
      ...
      stage = 'SESSION_DECRYPT';
      session = decryptSessionForRequest(scope,row,keyring,now().getTime(),context);
      decrypt = 'PASS';validation = 'PASS';
      const http = createYandexReadTransport({scope,session,allowRead,signal:deadline,context,
        observeNetwork:diagnostic=>{networkDiagnostic=diagnostic;},
      ...
      if(persist){
        stage='PERSISTENCE';
        const projected=reviews.map(review=>toReviewExternalReview(review,{companyId:scope.companyId,
          locationId:scope.locationId,observedAt:now().toISOString()}));
        persistenceStatus='ATTEMPTED';
        persistenceResult=await persistenceWriter.persistNormalizedReviews({companyId:scope.companyId,
          locationId:scope.locationId,provider:'yandex',externalLocationId:scope.organizationId,reviews:projected});
      ...
      const extra=['SESSION_REQUEST_INVALID','LIVE_READ_NOT_APPROVED','SESSION_NOT_READY','SESSION_MODE_INVALID',
        'REVIEW_SCOPE_COLLISION','REVIEW_BATCH_DUPLICATE','REVIEW_BATCH_INVALID','REVIEW_ROW_INVALID',
        'REVIEW_RAW_PAYLOAD_INVALID','REVIEW_STORAGE_FAILED','REVIEW_PHASE_INVALID','REVIEW_WRITER_CONTRACT_DRIFT'];
  ...
  return {
    contractDiagnosticFull:(input,options)=>completeRead(input,options),
    persistManual:async(input,{expectedRevision}={})=>{
      serverOnly(context);
      if(!context||allowManualPersistence!==true||!persistenceWriter||
         !Number.isSafeInteger(expectedRevision)||expectedRevision<1)fail('SESSION_MODE_INVALID');
      return completeRead(input,{expectedRevision,paginationMode:'MUTABLE_OFFSET'},true);
    },

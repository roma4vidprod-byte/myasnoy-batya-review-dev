...
export function createYandexSessionService({ store, keyring, allowRead = false, fetchImpl, notify = sendSessionAlert, now = () => new Date() }) {
  ...
  async function run(input, { mode = 'health', pageBase, maxPages = 20, safeReport = false } = {}) {
    serverOnly();
    const scope = scopeOf(input), numbering = createYandexPageNumbering(pageBase);
    if (!['health','dry_run','probe','pagination_probe'].includes(mode)) fail('SESSION_MODE_INVALID');
    ...
    let transportCode, paginationProbe, dryRunReport;
    ...
    try {
      ...
      const transport = async request => {
        try {
          ...
          observe(payload, evidence); audit?.observe(payload,request.page); return payload;
        } catch (error) { transportCode = error.code; throw error; }
      };
      ...
      return { ok: true, ...metadata(next), mode, seen, newCount, evidence, pageEvidence,

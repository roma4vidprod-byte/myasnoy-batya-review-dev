...
// Response metadata is also untrusted at this boundary. Only fixed rule paths
// and descriptions may leave the server, never arbitrary strings on an error.
const RULE_METADATA = Object.freeze({
  "SESSION_MATERIAL_SHAPE_INVALID": [
    "material",
    "object"
  ],
  "SESSION_MATERIAL_FIELDS_INVALID": [
    "material",
    "account,cookies only"
  ],
  "SESSION_MATERIAL_ACCOUNT_INVALID": [
    "account",
    "fixed account string"
  ],
  "SESSION_COOKIE_COUNT_INVALID": [
    "cookies",
    "non-empty array with at most 100 items"
  ],
  "SESSION_COOKIE_SHAPE_INVALID": [
    "cookies[*]",
    "object"
  ],
  "SESSION_COOKIE_FIELDS_INVALID": [
    "cookies[*]",
    "allowlisted cookie fields"
  ],
  "SESSION_COOKIE_NAME_INVALID": [
    "cookies[*].name",
    "safe cookie-name string"
  ],
  "SESSION_COOKIE_PROHIBITED": [
    "cookies[*].name",
    "non-prohibited cookie name"
  ],
  "SESSION_COOKIE_VALUE_INVALID": [
    "cookies[*].value",
    "bounded printable string"
  ],
  "SESSION_COOKIE_DOMAIN_INVALID": [
    "cookies[*].domain",
    "yandex.ru domain string"
  ],
  "SESSION_COOKIE_PATH_INVALID": [
    "cookies[*].path",
    "allowlisted sprav path string"
  ],
  "SESSION_COOKIE_SECURE_INVALID": [
    "cookies[*].secure",
    "true boolean"
  ],
  "SESSION_COOKIE_HTTPONLY_INVALID": [
    "cookies[*].httpOnly",
    "boolean when present"
  ],
  "SESSION_COOKIE_EXPIRY_INVALID": [
    "cookies[*].expires",
    "null|-1|finite unix-seconds number"
  ],
  "SESSION_COOKIE_EXPIRED": [
    "cookies[*].expires",
    "future unix-seconds number"
  ],
  "SESSION_COOKIE_DUPLICATE": [
    "cookies[*].name",
    "unique cookie-name string"
  ],
  "SESSION_MATERIAL_TOO_LARGE": [
    "material",
    "UTF-8 JSON <= 60000 bytes"
  ]
});
const RULE_TYPES = new Set(['undefined','null','array','object','boolean','number','string','bigint','symbol','function']);
const ownValue = (value, key) => {
  if (value === null || (typeof value !== 'object' && typeof value !== 'function')) return undefined;
  try { const d=Object.getOwnPropertyDescriptor(value,key);return d && Object.hasOwn(d,'value')?d.value:undefined; }
  catch { return undefined; }
};
export const safeSchemaRule = value => {
  const rule=ownValue(value,'rule') ?? value;
  const code=ownValue(rule,'code'),meta=typeof code==='string'&&Object.hasOwn(RULE_METADATA,code)?RULE_METADATA[code]:null;
  const actual=ownValue(rule,'actual_type'),present=ownValue(rule,'present');
  if(!meta || !SESSION_VALIDATION_RULE_CODES.has(code) || ownValue(rule,'path')!==meta[0] ||
    ownValue(rule,'expected_type')!==meta[1] || !RULE_TYPES.has(actual) || typeof present!=='boolean')return null;
  return {code,path:meta[0],expected_type:meta[1],actual_type:actual,present};
};

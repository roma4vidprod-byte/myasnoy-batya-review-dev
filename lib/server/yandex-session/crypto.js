...
export const FAILURE_CODES = new Set([
  ...
  'PAGE_BASE_MISMATCH','REVIEW_SCOPE_COLLISION','SESSION_OPERATION_FAILED'
]);
...
export function decryptSessionClassified(scope, stored, keyring, now = Date.now()) {
  serverOnly();
  let plaintext;
  try {
    ...
    let associatedData;
    try { associatedData = aad(scope, stored?.credential_version); }
    ...
    try { return validateSession(parsed, now); }
    catch { fail('SESSION_PLAINTEXT_SCHEMA_INVALID'); }
  } catch (error) {
...
export function decryptSession(scope, stored, keyring, now = Date.now()) {
  try { return decryptSessionClassified(scope, stored, keyring, now); }
  catch (error) {

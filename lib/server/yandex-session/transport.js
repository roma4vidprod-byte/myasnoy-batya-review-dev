...
export function createYandexReadTransport({ scope, session, allowRead = false, fetchImpl = (...args) => globalThis.fetch(...args), signal, context, observeSize }) {
  serverOnly(context);
  ...
  return async function read(request) {
    ...
    try {
      response = await fetchImpl(url, {
        ...
        redirect: 'manual', signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(10000)]) : AbortSignal.timeout(10000)
      });

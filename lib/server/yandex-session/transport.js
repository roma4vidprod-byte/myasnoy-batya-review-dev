...
import { safeNetworkDiagnostic } from './network-diagnostic.js';

export function createYandexReadTransport({ scope, session, allowRead = false, fetchImpl = (...args) => globalThis.fetch(...args), signal, context, observeSize, observeNetwork }) {
  serverOnly(context);
  const target = scopeOf(scope);
  if (context) assertContextScope(context, target);
  return async function read(request) {
    serverOnly(context);
    if (allowRead !== true) fail('LIVE_READ_NOT_APPROVED');
    const page = request?.page;
    if (!Number.isSafeInteger(page) || page < (context ? 1 : 0) || page > (context ? 10 : 100)) fail('SESSION_REQUEST_INVALID');
    ...
    const networkFailure=(error,phase)=>{
      const diagnostic=safeNetworkDiagnostic(error,{phase,aborted:requestSignal.aborted});
      // Diagnostics cannot throw raw observer errors or change fail-closed behavior.
      try { if(typeof observeNetwork==='function')observeNetwork(diagnostic); } catch {}
      fail('YANDEX_NETWORK_ERROR');
    };
    let response;
    try {
    ...
    } catch (error) { networkFailure(error,'REQUEST_HEADERS'); }
    ...
    try {
      ...
      if (typeof observeSize === 'function') observeSize(size);
    } catch (error) {
      if(['YANDEX_RESPONSE_TOO_LARGE','YANDEX_MALFORMED_JSON'].includes(error.code))fail(error.code);
      networkFailure(error,'RESPONSE_BODY');
    }

...
export function createReviewContractDiagnostic({
  ...
  serverOnly();
  return async function runDiagnostic(page = 1) {
    const before = await preflight();
    const base = safePreflightResult(before, 'contract_diagnostic');
    ...
    try {
      ...
      const result = await service.contractDiagnostic(ASBEST_SYNC_SCOPE, { page });
      return {
        ...result,
        operation: 'contract_diagnostic',
        pretransport: 'PASS', decrypt: 'PASS', validation: 'PASS'
      };
    } catch (error) {
      return {
        ok: false, operation: 'contract_diagnostic', pretransport: 'PASS', decrypt: 'PASS', validation: 'PASS',
  ...
  return async function handler(req, res) {
    ...
    if (req.body && Object.prototype.hasOwnProperty.call(req.body, 'operation')) {
      ...
      const allowedKeys = operation === 'import_prepare' ? ['operation'] :
        ...
        operation === 'contract_diagnostic_page' ? ['operation', 'page'] : ['operation'];
      if (!['preflight', 'health', 'contract_diagnostic', 'contract_diagnostic_page', 'reconcile_connection', 'import_prepare', 'import'].includes(operation) ||
      ...
      try {
        const result = operation === 'preflight' ? safePreflightResult(await preflight()) :
          operation === 'health' ? await health() : operation === 'contract_diagnostic' ? await contractDiagnostic() :
        ...
        if (operation === 'reconcile_connection') {
          return res.status(503).json({ ok: false, operation, error: 'RECONCILIATION_FAILED' });
        }

...
export function createReviewContractDiagnostic({
  ...
  serverOnly();
  return async function runDiagnostic() {
    const before = await preflight();
    ...
    try {
      ...
      const result = await service.contractDiagnostic(ASBEST_SYNC_SCOPE);
      return {
        ...result,
...
export function createReviewSyncWorkerHandler({
  ...
  return async function handler(req, res) {
    ...
    if (req.body && Object.prototype.hasOwnProperty.call(req.body, 'operation')) {
      ...
      if (!['preflight', 'health', 'contract_diagnostic', 'reconcile_connection', 'import_prepare', 'import'].includes(operation) ||
      ...
      try {
        const result = operation === 'preflight' ? safePreflightResult(await preflight()) :
          operation === 'health' ? await health() : operation === 'contract_diagnostic' ? await contractDiagnostic() : operation === 'reconcile_connection' ?

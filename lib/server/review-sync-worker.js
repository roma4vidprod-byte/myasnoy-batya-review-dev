import { reconciliationFailureResult } from './reconciliation-result.js';
import { singleSyncRun } from './single-sync-run.js';
...
import { createYandexConnectionReconciliation, requestDevServiceRpc } from './review-sync.js';
import { createYandexSessionRuntimeStatus } from './yandex-session/runtime-status.js';
import { runYandexHealthPreflight, safeSchemaRule } from './yandex-session/preflight.js';
...
const safeFailureCodes = new Set([...FAILURE_CODES, 'REVIEW_WRITER_CONTRACT_DRIFT', 'SYNC_OPERATION_FAILED', 'INTERNAL_FAILURE']);
const safeHealthStages = new Set(['DEPENDENCY_INIT','SERVICE_INIT','SERVICE_RUN','SESSION_READ','SESSION_GUARD',
  'SESSION_DECRYPT','REQUEST_PREPARE','SESSION_RECHECK','TRANSPORT','PARSER','PERSISTENCE','HEALTH_CAS','HEALTH_COMPLETE']);
const safeCasStatuses = new Set(['NOT_ATTEMPTED','SUCCESS','FAILED','CONFLICT']);
...
function safePreflightResult(result, operation = 'preflight') {
  return {
    ...
    decrypt: result.session_decrypt ?? (result.pretransport === 'PASS' ? 'PASS' : 'NOT_RUN'),
    validation: result.session_validation ?? (result.session_decrypt === 'PASS' ? 'PASS' : 'NOT_RUN'),
    decrypt_stage: result.decrypt_stage ?? null,
    schema_rule: result.error === 'SESSION_PLAINTEXT_SCHEMA_INVALID' ? safeSchemaRule(result.schema_rule) : null,
...
export function createReviewSyncHealth({
  ...
  return async function runHealth() {
    ...
    let keyring;
    let yandexRequests = 0, transportCompleted = 0, providerHttpStatus = null;
    let stage = 'DEPENDENCY_INIT';
    try {
      keyring = keyringFactory();
      stage = 'SERVICE_INIT';
      const service = serviceFactory({
        ...
        fetchImpl: async (...args) => {
          stage = 'TRANSPORT'; yandexRequests += 1;
          const response = await fetchImpl(...args);
          if (response && Number.isSafeInteger(response.status)) {
            transportCompleted += 1; providerHttpStatus = response.status;
          }
          return response;
        }, now
      });
      ...
      stage = result.ok === true ? 'HEALTH_COMPLETE' : (result.failureStage ?? 'SERVICE_RESULT');
      const response = {
        ...
        state_after: result.state ?? 'NOT_CONFIRMED', revision_after: result.revision ?? 'NOT_CONFIRMED',
        last_session_check_at: result.lastSessionCheckAt ?? null,
        last_successful_sync_at: result.lastSuccessfulSyncAt ?? null,
        provenance: before.provenance ? { ...before.provenance, decrypt: result.failureStage === 'SESSION_DECRYPT' ? 'FAIL' : 'PASS' } : null,
        seen: result.ok === true ? result.seen : 'NOT_RETURNED',
        pages_fetched: result.ok === true ? result.pagesFetched : 'NOT_RETURNED',
        yandex_requests: yandexRequests, transport_attempted: yandexRequests,
        transport_completed: transportCompleted, provider_http_status: providerHttpStatus ?? 'NOT_RETURNED',
        failure_stage: result.failureStage ?? null, last_completed_stage: result.lastCompletedStage ?? 'NOT_RETURNED',
        health_cas: result.stateCas ?? 'NOT_RETURNED', review_persistence: 'OFF',
        ...(result.ok === true ? {} : { error: safeCode(result.errorCode) })
      };
      return response;
    } catch (error) {
      const errorCode = safeFailureCodes.has(error?.code) ? error.code : 'INTERNAL_FAILURE';
      const failureStage = safeHealthStages.has(error?.failureStage) ? error.failureStage : stage;
      const lastCompletedStage = safeHealthStages.has(error?.lastCompletedStage) ? error.lastCompletedStage : 'NOT_RETURNED';
      const healthCas = safeCasStatuses.has(error?.stateCas) ? error.stateCas : 'NOT_ATTEMPTED';
      return {
        ok: false, operation: 'health', pretransport: 'PASS', decrypt: 'PASS', validation: 'PASS',
        health_get: yandexRequests > 0 ? 'FAIL' : 'NOT_STARTED',
        state_before: before.session_state, revision_before: before.revision,
        state_after: 'NOT_CONFIRMED', revision_after: 'NOT_CONFIRMED', last_session_check_at: null,
        last_successful_sync_at: null, seen: 'NOT_RETURNED', pages_fetched: 'NOT_RETURNED',
        provenance: before.provenance ? { ...before.provenance, decrypt: 'PASS' } : null,
        yandex_requests: yandexRequests, transport_attempted: yandexRequests,
        transport_completed: transportCompleted, provider_http_status: providerHttpStatus ?? 'NOT_RETURNED',
        failure_stage: failureStage, last_completed_stage: lastCompletedStage, health_cas: healthCas,
        review_persistence: 'OFF', error: errorCode
      };
    } finally {
...
export function createReviewContractDiagnostic({
  preflight = runYandexHealthPreflight,
  keyringFactory = keyringFromEnv,
  serviceFactory = createYandexSessionService,
  storeFactory = () => createSessionStore(),
  ...
  serverOnly();
  return async function runDiagnostic(page = 1, { full = false, expectedRevision } = {}) {
    const operation = full ? 'contract_diagnostic_full' : 'contract_diagnostic';
    const before = await preflight();
    const base = safePreflightResult(before, operation);
    if (!base.ok) return { ...base, schema: null, parser: null, pagination: null, yandex_requests: 0, review_persistence: 'OFF' };
    let keyring;
    try {
      keyring = keyringFactory();
      const service = serviceFactory({
        store: storeFactory(), keyring, allowRead: allowRead(),
        fetchImpl, now
      });
      const result = full ? await service.contractDiagnosticFull(ASBEST_SYNC_SCOPE, { expectedRevision }) :
        await service.contractDiagnostic(ASBEST_SYNC_SCOPE, { page });
      return {
        ...result,
        operation,
        pretransport: 'PASS', decrypt: result.decrypt ?? 'PASS', validation: result.validation ?? 'PASS'
      };
    } catch (error) {
      return {
        ok: false, operation, pretransport: 'PASS', decrypt: 'PASS', validation: 'PASS',
        error: safeCode(error?.code), yandex_requests: 0, review_persistence: 'OFF'
      };
    } finally {
      clearKeyring(keyring);
    }
...
export function createReviewSyncWorker({
  ...
  return async function runOne() {
    ...
    return singleSyncRun({ rpc, companyId: ASBEST_SYNC_SCOPE.companyId, validateClaim,
      safeCode, errorState: code => reauthCodes.has(code) ? 'REAUTH_REQUIRED' : 'ERROR',
      summarize: safeResult, processClaim: async () => {
        ...
        return service.run(ASBEST_SYNC_SCOPE, {
...
export function createReviewSyncWorkerHandler({
  ...
  contractDiagnostic = createReviewContractDiagnostic(),
  reconcileConnection = createYandexConnectionReconciliation(),
  importPrepare,
  importSession
} = {}) {
  ...
  return async function handler(req, res) {
    ...
    if (req.body && Object.prototype.hasOwnProperty.call(req.body, 'operation')) {
      ...
      const allowedKeys = operation === 'import_prepare' ? ['operation'] :
        operation === 'import' ? ['operation', 'capability', 'nonce', 'session'] :
        operation === 'contract_diagnostic_page' ? ['operation', 'page'] :
          operation === 'contract_diagnostic_full' ? ['operation', 'expected_revision'] : ['operation'];
      if (!['preflight', 'health', 'contract_diagnostic', 'contract_diagnostic_page', 'contract_diagnostic_full', 'reconcile_connection', 'import_prepare', 'import'].includes(operation) ||
          (operation === 'contract_diagnostic_page' && (!Number.isSafeInteger(req.body.page) || req.body.page < 2 || req.body.page > 4)) ||
          (operation === 'contract_diagnostic_full' && (!Number.isSafeInteger(req.body.expected_revision) || req.body.expected_revision < 0)) ||
      ...
      try {
        const result = operation === 'preflight' ? safePreflightResult(await preflight()) :
          operation === 'health' ? await health() : operation === 'contract_diagnostic_full' ?
            await contractDiagnostic(1, { full: true, expectedRevision: req.body.expected_revision }) : operation === 'contract_diagnostic' ? await contractDiagnostic() :
            operation === 'contract_diagnostic_page' ? await contractDiagnostic(req.body.page) : operation === 'reconcile_connection' ?
            await reconcileConnection() : operation === 'import_prepare' ?
            await prepareImport() : await importRemoteSession(req.body);
        return res.status(result.ok ? 200 : 503).json(result);
      } catch (error) {
        if (operation === 'reconcile_connection') {
          return res.status(503).json({ ...reconciliationFailureResult(error), operation });
        }

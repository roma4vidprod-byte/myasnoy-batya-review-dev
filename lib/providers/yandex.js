...
const DIAGNOSTIC_FAILURE_FIELDS = new Set([
  'response.error', 'list', 'list.items', 'list.pager', 'list.pager.limit',
...
export function inspectYandexReviewsPayload(payload) {
  ...
  return {
    top_level_keys: safeKeys(response),
    list_keys: safeKeys(list),
    pager_keys: safeKeys(pager),
    item_keys: itemKeys,
    author_keys: authorKeys,
    owner_comment_keys: ownerCommentKeys,
    unexpected_keys: {
      top_level: safeKeys(response).filter(key => key !== 'list'),
      list: safeKeys(list).filter(key => !['items', 'pager'].includes(key)),
      pager: safeKeys(pager).filter(key => !['limit', 'offset', 'total'].includes(key)),
      item: itemKeys.filter(key => !DIAGNOSTIC_ITEM_TOP_LEVEL_FIELDS.has(key)),
      author: authorKeys.filter(key => key !== 'user'),
      owner_comment: ownerCommentKeys.filter(key => !['text', 'time_created', 'moderation_status'].includes(key))
    },
...
export function diagnoseYandexReviewsPayload(payload, externalLocationId = null) {
  const schema = inspectYandexReviewsPayload(payload);
  try {
    const parsed = parseYandexReviewsPayload(payload, externalLocationId);
    return {
      ok: true,
    ...
    return {
      ...
      parser: {
        code,
        failure_point: DIAGNOSTIC_FAILURE_FIELDS.has(error?.field) ? error.field : 'unknown'
      },

...
function timestamp(value, field) {
  if (value === null) return null; // Both provider time types remain TYPE CONFIRMATION PENDING.
  let milliseconds;
  if (typeof value === 'number' || (typeof value === 'string' && /^\d+$/.test(value))) {
    const number = Number(value);
    if (!Number.isSafeInteger(number) || number < 0) drift(field);
    // Explicitly supported fixture formats: Unix seconds / modern Unix milliseconds.
...
export async function fetchYandexReviews({ permanentId, transport, maxPages = 20, pageBase = 1 } = {}) {

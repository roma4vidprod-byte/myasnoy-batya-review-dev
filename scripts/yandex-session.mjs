...
import { runYandexHealthPreflight } from '../lib/server/yandex-session/preflight.js';

try {
  if (process.argv.length !== 3 || !['status','import','preflight','health','probe','dry-run','rotate-key','disable'].includes(process.argv[2])) fail('CLI_OPERATION_REQUIRED');
  ...
  if (op === 'preflight') {
    const result = await runYandexHealthPreflight({ scope: data.scope, pageBase: data.pageBase });
    process.stdout.write(JSON.stringify(result) + '\n');
    if (result.pretransport !== 'PASS') process.exitCode = 1;
  } else {

...
> **Native/Windows gate checkpoint 2026-09-14:** current `c47b83b179b9a94781d55029b4b399c44f9a075b` already contains the ready patch; forward apply was not repeated and reverse apply check passed. Actual Windows results: `npm test` 489/489 PASS, `npm run check` 107/107 PASS, `git diff --check` PASS, PowerShell 7.6.5. Native PostgreSQL 17 concurrency is `NOT_RUN`: the prepared runner path and `initdb`/`pg_ctl`/`psql` are absent in this checkout/host. Full evidence: `docs/NATIVE_WINDOWS_GATE_20260914.md`. The pre-existing untracked operator script was preserved and not committed.

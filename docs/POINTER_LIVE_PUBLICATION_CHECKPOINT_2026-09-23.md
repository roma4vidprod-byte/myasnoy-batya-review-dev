...
Следующее действие пользователя: открыть именно почту, указанную в этом проходе, и оставить её активной; проверять только письмо Pointer/«Мясного Бати», не содержимое других писем. Повторять клиентский проход не требуется.

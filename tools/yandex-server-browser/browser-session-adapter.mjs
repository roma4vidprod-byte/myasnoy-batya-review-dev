...
function credentialPath(env=process.env){
  ...
  if(typeof dir!=='string'||!dir.startsWith('/run/credentials/'))fail('SESSION_KEY_NOT_CONFIGURED');
  return join(dir,'yandex-session-key');
}
export function loadBrowserKeyring(env=process.env){
  ...
  try{
    ...
    if(!s.isFile()||s.uid!==0||s.gid!==0||s.nlink!==1||s.size<40||s.size>1024||
       (s.mode&0o777)!==0o440)fail('SESSION_KEY_NOT_CONFIGURED');
    raw=readFileSync(fd);
    const value=JSON.parse(raw.toString('utf8'));
    if(Object.keys(value).sort().join()!=='key,kid'||
       !/^vps-yandex-[a-f0-9]{16}$/.test(value.kid)||
       !/^[A-Za-z0-9+/]{43}=$/.test(value.key))fail('SESSION_KEY_NOT_CONFIGURED');
    const key=Buffer.from(value.key,'base64');value.key='';
...
export function createBrowserSessionAdapter({store,context,now=()=>Date.now(),loadKeyring=loadBrowserKeyring}={}){
  if(!store||typeof store.readSession!=='function')fail('BROWSER_SESSION_STORE_INVALID');

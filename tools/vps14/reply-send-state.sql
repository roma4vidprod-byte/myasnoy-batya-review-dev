...
begin
  if not exists(select 1 from pg_roles where rolname='review-yandex-writer') then
    create role "review-yandex-writer" login nosuperuser nocreatedb nocreaterole noinherit nobypassrls;
  end if;
end
$$;
...
create or replace function vps_yandex_private.reply_worker_call(
  p_operation text,
  p_action_id uuid,
...
begin
  ...
  if p_action_id is null or p_idempotency_key is null or
     p_operation not in ('claim','complete','fail') then
  ...
  select * into v_review from public.review_external_reviews
  where id=v_action.external_review_row_id for update;

  if p_operation='claim' then
    if v_action.status <> 'QUEUED' then
      return jsonb_build_object('claimed',false,'status',v_action.status);
    end if;
    if v_action.approval_fingerprint is null or v_action.approved_at is null
       or v_action.approval_expires_at is null or
       v_action.approval_expires_at < v_now then
      ...
      if v_review.owner_reply_text is null then
        update public.review_external_reviews set reply_state='FAILED'
        where id=v_review.id and reply_state='QUEUED';
      end if;
      return jsonb_build_object('claimed',false,'status','FAILED','error','REPLY_APPROVAL_EXPIRED');
    end if;
    if v_review.owner_reply_text is not null then
      ...
      set status='FAILED',last_error='REVIEW_ALREADY_ANSWERED',
          finished_at=v_now,updated_at=v_now
      where id=v_action.id and status='QUEUED';
      update public.review_external_reviews set reply_state='SYNCED_EXTERNAL'
      where id=v_review.id;
      return jsonb_build_object('claimed',false,'status','FAILED','error','REVIEW_ALREADY_ANSWERED');
    end if;
    ...
    if not found then return jsonb_build_object('claimed',false,'status','CONFLICT'); end if;

    return jsonb_build_object(
      'claimed',true,'status','SENDING',
      ...
      'approval_expires_at_ms',floor(extract(epoch from v_action.approval_expires_at)*1000)::bigint,
  ...
  if p_operation='complete' then
    update public.review_reply_actions
    set status='SENT',sent_at=v_now,finished_at=v_now,last_error=null,updated_at=v_now
    where id=v_action.id and status='SENDING' and idempotency_key=p_idempotency_key;
    update public.review_external_reviews
    set reply_state=case when owner_reply_text is null then 'SENT' else 'SYNCED_EXTERNAL' end
    where id=v_review.id;
    return jsonb_build_object('status','SENT','action_id',v_action.id);
  end if;
  ...
  update public.review_reply_actions
  set status='FAILED',last_error=v_error,finished_at=v_now,updated_at=v_now
  where id=v_action.id and status='SENDING' and idempotency_key=p_idempotency_key;
  update public.review_external_reviews
  set reply_state=case when owner_reply_text is null then 'FAILED' else 'SYNCED_EXTERNAL' end
  where id=v_review.id;
  return jsonb_build_object('status','FAILED','action_id',v_action.id,'error',v_error);
end
$$;

revoke all on function vps_yandex_private.reply_worker_call(text,uuid,uuid,text)
from public,anon,authenticated,service_role;
grant execute on function vps_yandex_private.reply_worker_call(text,uuid,uuid,text)
